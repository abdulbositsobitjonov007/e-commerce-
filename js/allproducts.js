...
let otherCarts = JSON.parse(localStorage.getItem("cart") || "[]");

localStorage.setItem("cart", JSON.stringify(otherCarts));

function getAllProducts(content, data) {
    content.innerHTML = "";
    data.map((el) => {
        content.innerHTML +=
            `
                    ...
                    : el.rating === 4 ?
                        ...
                        : el.rating === 3.5 ?
                            ...
                            : el.rating === 3 ?
                                ...
                                ${(isExist = otherCarts.find((car) => car.id === el.id))
...
function addtoCart(id) {
    ...
    item.qty = 1;
    otherCarts.push(item);
    localStorage.setItem("cart", JSON.stringify(otherCarts));
    badge.textContent = otherCarts.length;
    getAllProducts(allProducts, products);
...
function increaseBtn(id) {
    let item = otherCarts.find((el) => el.id === id)
    item.qty += 1;
    localStorage.setItem("cart", JSON.stringify(otherCarts));
    getAllProducts(allProducts, products);
}
...
function decreaseBtn(id) {
    ...
    if (item.qty <= 0) {
        otherCarts = otherCarts.filter((el) => el.id !== id)
        badge.textContent = otherCarts.length;
        localStorage.setItem("cart", JSON.stringify(otherCarts));
    }
    getAllProducts(allProducts, products);
